...
Custom React hooks for blockchain interaction, state management, and data fetching. All hooks live under `Project/frontend/src/hooks/`. All hooks are built on **wagmi v3** and **TanStack React Query v5** for automatic caching, refetching, and optimistic updates.
...
|------|---------|-------------|-----------------|
| [`useUserRegistry`](#useuserregistry) | `role`, `isClient`, `isFreelancerOwner`, `registerUser(role)` | Check user role, register new users | `UserRegistry` |
| [`useAgentRegistry`](#useagentregistry) | `agent`, `isLoading`, `error` | Get single agent profile (ERC-7857) | `AgentRegistry` |
| [`useAgentERC7857`](#useagenterc7857) | `updateCapability()`, `authorizeUsage()`, `iTransfer()`, `iClone()`, `transferDigest()`, `authorizedUsersOf()` | ERC-7857 specific actions | `AgentRegistry` |
| [`useAgentManagement`](#useagentmanagement) | `mintAgent()`, `toggleActive()`, `updateProfileHash()`, `addSkill()`, `removeSkill()`, `updateCapability()` | Create/edit agents | `AgentRegistry` |
| [`useAllAgents`](#useallagents) | `agents`, `isLoading`, `refetch`, `hasMore` | List all agents (marketplace) | `AgentRegistry` |
| [`useProgressiveEscrow`](#useprogressiveescrow) | `postJob()`, `submitProposal()`, `acceptProposal()`, `defineMilestones()`, `releaseMilestone()`, `cancelJob()`, `cancelStaleJob()`, `getJob()`, `getProposals()` | ERC-8183 job lifecycle | `ProgressiveEscrow` |
| [`useSubscriptionEscrow`](#usesubscriptionescrow) | `createSubscription()`, `topUp()`, `cancelSubscription()`, `approveInterval()`, `proposeInterval()`, `updateInterval()`, `drainPerCheckIn()`, `drainPerAlert()`, `setWebhookHash()`, `finalizeExpired()`, `getSubscription()` | Subscription operations | `SubscriptionEscrow` |
...
| `registerUser(role)` | `(role: 1 \| 2) => Promise<void>` | Register caller with chosen role (Client=1, FreelancerOwner=2) | Call on role selection |

### Contract Called

| Contract | Function | Purpose |
|----------|----------|---------|
| `UserRegistry` | `registerUser(Role role)` | Register wallet with role (1=Client, 2=FreelancerOwner). One-time per wallet. |
| `UserRegistry` | `getUserRole(address) → Role` | Read user's role from contract |
| `UserRegistry` | `isRegistered(address) → bool` | Convenience helper |
...
function RoleSelector() {
  const { role, isClient, isFreelancerOwner, registerUser, isLoading } = useUserRegistry();
  ...
  if (role === 'Unregistered') {
    return (
      <div>
        <button onClick={() => registerUser(1)}>I want to hire agents</button>
        <button onClick={() => registerUser(2)}>I want to own agents</button>
      </div>
    );
  }
...
| Contract | Function | Purpose |
|----------|----------|---------|
| `AgentRegistry` | `mintAgent(...)` | Mint new agent iNFT (**ERC-7857**) |
...
| `releaseMilestone()` | `jobId: number, index: number, outputCID: string, score: number` | `Promise<void>` | Agent submits work |
...
| `ProgressiveEscrow` | `releaseMilestone(uint256 jobId, uint256 index, string outputCID, uint256 score)` | Submit work |
