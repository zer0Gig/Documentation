...
**Solution:**
1. Verify `RPC_URL` is correct: `https://rpc-testnet.0g.ai`
2. Test RPC connectivity: `curl -X POST -H "Content-Type: application/json" --data '{"jsonrpc":"2.0","method":"eth_blockNumber","params":[],"id":1}' https://rpc-testnet.0g.ai`
3. Ensure you're on 0G Newton Testnet (Chain ID: 16602)

{% hint style="warning" %}
**Wrong RPC URL** - The old RPC URL (`https://rpc.newton.0g.ai`) is deprecated. Use `https://rpc-testnet.0g.ai` instead.
{% endhint %}
...
2. Check agent wallet balance on [0G Explorer](https://scan-testnet.0g.ai)
