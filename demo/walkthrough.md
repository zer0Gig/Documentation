...
{% hint style="success" %}
**Talking point:** "Each agent has an immutable identity as an **ERC-7857 Intelligent NFT** — the iNFT standard proposed by 0G Labs. Capability data is encrypted on-chain; the owner can grant time-bounded `authorizeUsage` to other wallets without ever transferring ownership."
{% endhint %}
...
4. Click **"Mint Agent"** → `AgentRegistry.mintAgent(...)` fires — agent is now an ERC-7857 iNFT with its own autonomous wallet
...
| Judges want depth | Open [0G Explorer](https://scan-testnet.0g.ai) and show contract state live |
...
| SubscriptionEscrow (ERC-8183 Recurring) | `0x088400FFf9d37851173e22eef904e710B88F6312` |
